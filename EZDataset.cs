...
namespace EZForms
{
    ...
    public class EZDataset : IDisposable
    {
        ...
        public EZDataset(string database, string source, Dictionary<string, object> parameters = null, string order = null, int size = 1)
        {
            ...
            {
                Debug.WriteLine("EZDataset: null database name!");
                return;
            }

            if (string.IsNullOrWhiteSpace(source))
            {
                Debug.WriteLine("EZDataset: null data source!");
                return;
            }
            ...
            if (this.size < 0)
            {
                Debug.WriteLine($"EZDataset: [{database}].[{source}] - Page size should not be less than zero! Coerced to zero (all records will be fetch).");
                this.size = 0;
            }
        ...
        {
            Disconnect();
            Debug.WriteLine($"EZDataset: [{database}].[{source}] Finalized!");
        }

        public void Dispose()
        {
            Disconnect();
            Debug.WriteLine($"EZDataset: [{database}].[{source}] Disposed!");
        }
        ...
        public bool FetchPage()
        {
            ...
            if ((connection != null) && (command != null))
            {
                ...
                {
                    Debug.WriteLine("Failed to fetch page!");
                    Debug.WriteLine(e.Message);
                }
            }
        ...
        private void Build()
        {
            ...
            if (connection == null)
            {
                Debug.WriteLine("EZDataset: Could not establish a connection to [{database}]");
                return;
            }
            ...
            {
                Debug.WriteLine($"EZDataset: Could not build [{database}].[{source}]");
                Debug.WriteLine(e.Message);
            }
        }
        ...
        private void Disconnect()
        {
            ...
            {
                Debug.WriteLine($"EZDataset: Failed to dispose [{database}].[{source}]");
            }
        }
        ...
        private string OrderBy()
        {
            ...
            if (!string.IsNullOrWhiteSpace(order))
            {
                ...
                foreach (string token in tokens)
                {
                    ...
                    Debug.WriteLine($"No such column: [{column}] for ORDER BY on [{database}].[{source}");
                }
